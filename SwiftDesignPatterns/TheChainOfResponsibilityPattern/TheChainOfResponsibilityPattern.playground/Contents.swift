    ...
    func handle(request: Any) {
        if request is T {
            ...
            guard let handler = nextHandler else {
                print("Reached the end of responder chain")
                return
            }
            
            handler.handle(request: request)
            print("\(self) can't handle \(T.self) request - forwarding to \(handler)")
        }
    }
...
dateHandler.handle(request: 42) // As in responder chain there is no integer handler so request reached to end of responder chain without process our request
